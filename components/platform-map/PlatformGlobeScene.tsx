...
function Globe({ reducedMotion, worldRef, globeRef }: { reducedMotion: boolean; worldRef: React.RefObject<Group | null>; globeRef: React.RefObject<Mesh | null> }) {
  ...
  return <><mesh ref={globeRef} scale={GLOBE_RADIUS}><sphereGeometry args={[1, 128, 128]} /><meshPhysicalMaterial map={texture} normalMap={normalMap} normalScale={[.8, .8]} emissiveMap={texture} emissive="#ffad3d" emissiveIntensity={.28} metalness={.05} roughness={.72} clearcoat={.25} /></mesh><CityLights /><Atmosphere /></>
}
...
export function PlatformGlobeScene({ sections }: { sections: PlatformHotspot[] }) {
  ...
  return <Canvas style={{ width: "100%", height: "100%", background: "transparent" }} camera={{ position: [0, 0, 18], fov: 30 }} gl={{ alpha: true, antialias: true, powerPreference: "high-performance" }} dpr={[1, 1.5]}><SkyParallax reducedMotion={reducedMotion} /><ambientLight intensity={.5} color="#101820" /><directionalLight position={[15, 8, 10]} intensity={3.5} color="#fff8e8" /><directionalLight position={[-10, -3, -8]} intensity={.3} color="#2a4a7a" /><directionalLight position={[-8, 4, -12]} intensity={1.5} color="#4da6ff" /><CameraDolly /><Suspense fallback={null}><group ref={worldRef} rotation={[0, 0, 23.5 * Math.PI / 180]}><Globe reducedMotion={reducedMotion} worldRef={worldRef} globeRef={globeRef} /><CloudLayer reducedMotion={reducedMotion} />{sections.map((section, i) => <Hotspot key={section.key} hotspot={section} radius={GLOBE_RADIUS + .02} occludeRef={globeRef} delayMs={i * 60} reducedMotion={reducedMotion} />)}</group></Suspense><OrbitControls enableDamping dampingFactor={.075} autoRotate={false} enablePan={false} minDistance={10} maxDistance={24} rotateSpeed={.5} /></Canvas>
}
