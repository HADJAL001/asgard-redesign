...
export function PlatformGlobeScene({ sections }: { sections: PlatformHotspot[] }) {
  ...
  return <Canvas style={{ width: "100%", height: "100%", background: "transparent" }} camera={{ position: [0, 0, 18], fov: 30 }} gl={{ alpha: true, antialias: true, powerPreference: "high-performance" }} dpr={[1, 1.5]}><SkyParallax reducedMotion={reducedMotion} /><directionalLight position={[10, 5, 8]} intensity={1.15} color="#dcecff" /><ambientLight intensity={.08} color="#315274" /><CameraDolly /><Suspense fallback={null}><group ref={worldRef} rotation={[0, 0, 23.5 * Math.PI / 180]}><Globe reducedMotion={reducedMotion} worldRef={worldRef} globeRef={globeRef} /><CloudLayer reducedMotion={reducedMotion} />{sections.map((section, i) => <Hotspot key={section.key} hotspot={section} radius={GLOBE_RADIUS + .08} occludeRef={globeRef} delayMs={i * 60} reducedMotion={reducedMotion} />)}</group></Suspense><OrbitControls enableDamping dampingFactor={.075} autoRotate={false} enablePan={false} minDistance={10} maxDistance={24} rotateSpeed={.5} /></Canvas>
}
