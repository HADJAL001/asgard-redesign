...
const CATEGORIES: Category[] = [
  ...
  { id: "orchestrator", label: "Оркестратор", Icon: Workflow },
...
const ARTICLES: Article[] = [
  ...
  {
    id: "orchestrator-guide",
    category: "orchestrator",
    categoryLabel: "Оркестратор",
    subLabel: "Руководства",
    title: "Как пользоваться оркестратором",
    description: "Сборка и запуск цепочек из нейросетей в визуальном редакторе.",
    views: 3,
    updated: "22.07.2026",
    toc: [
    ...
    sections: [
      {
        heading: "Что такое оркестратор",
        body: "Оркестратор — визуальный редактор, в котором можно собрать цепочку из нескольких нейросетей: выход одного узла становится входом следующего. Цепочка выполняется строго линейно, в порядке, заданном связями между узлами.",
      },
      {
        heading: "Создание цепочки",
        body: 'Откройте раздел «Оркестратор» и нажмите «Создать цепочку». Перетащите нужные узлы из палитры на холст: OSGARD 5.0, OSGARD 4.0, OSGARD 3.3 или Шаблон промпта. Каждому узлу можно задать системный промпт, температуру и лимит токенов.',
      },
      {
        heading: "Соединение узлов",
        body: "Соедините узлы стрелками в том порядке, в котором данные должны проходить через цепочку. Узел без входящей связи получает начальный текст, введённый перед запуском; узел без исходящей связи считается последним, и его ответ становится результатом всей цепочки.",
      },
      {
        heading: "Запуск цепочки",
        body: 'Сохраните цепочку кнопкой «Сохранить», введите входной текст в поле над холстом и нажмите «Запустить». Статус каждого узла обновляется в реальном времени: ожидание, выполнение, готово или ошибка.',
      },
      {
        heading: "Стоимость запуска",
        body: "Запуск цепочки списывает TimeCoin — стоимость зависит от моделей, задействованных в цепочке (OSGARD 5.0, OSGARD 4.0, OSGARD 3.3 имеют разную цену за вызов, узел «Шаблон промпта» бесплатен). Если баланса не хватает, редактор покажет предложение пополнить кошелёк.",
      },
    ],
  },
  {
  ...
  {
    id: "orchestrator-nodes",
    category: "orchestrator",
    categoryLabel: "Оркестратор",
    subLabel: "Разработка",
    title: "Как добавлять новые узлы",
    description: "Расширение палитры оркестратора новым типом узла (для разработчиков).",
    views: 1,
    updated: "22.07.2026",
    toc: [
      "Где описаны типы узлов",
      "Шаг 1: Тип узла",
      "Шаг 2: Палитра",
      "Шаг 3: Стоимость на бэкенде",
      "Шаг 4: Обработка при выполнении",
    ],
    sections: [
      {
        heading: "Где описаны типы узлов",
        body: "Типы узлов определены на клиенте в lib/orchestrator/types.ts (тип OrchestratorNodeType) и в палитре components/orchestrator/node-types.ts (ORCHESTRATOR_PALETTE). На бэкенде стоимость и логика выполнения узла заданы в orchestrator.service.ts.",
      },
      {
        heading: "Шаг 1: Тип узла",
        body: 'Добавьте новый вариант в union OrchestratorNodeType (например, "gemini"). Если узлу нужны специфичные параметры, расширьте интерфейс OrchestratorNodeData.',
      },
      {
        heading: "Шаг 2: Палитра",
        body: "Добавьте новый элемент в массив ORCHESTRATOR_PALETTE: иконку, цвет, ключи локализации для названия и описания (labelKey/descriptionKey) и defaultData — значения параметров узла по умолчанию. После этого узел появится в панели перетаскивания редактора.",
      },
      {
        heading: "Шаг 3: Стоимость на бэкенде",
        body: "Добавьте новый тип в карту NODE_COST_TC на бэкенде — она задаёт, сколько TimeCoin списывается за вызов узла этого типа при запуске цепочки.",
      },
      {
        heading: "Шаг 4: Обработка при выполнении",
        body: "Убедитесь, что executeChain() на бэкенде умеет обрабатывать новый тип узла: к какой модели/API обращаться и как формировать запрос из systemPrompt/temperature/maxTokens или template. Не забудьте добавить ключи локализации nodeType.<тип> и nodeType.<тип>Desc во все языковые файлы (ru/en/kz).",
      },
    ],
  },
  {
