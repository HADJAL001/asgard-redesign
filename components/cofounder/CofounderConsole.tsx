...
export function CofounderConsole() {
  ...
  return (
    <main className="ds-body" style={{ minHeight: "100vh", padding: "clamp(1rem, 4vw, 4rem)" }}>
      ...
      <dialog aria-label="НОВЫЙ КОНТРАКТ" open={open} onClose={() => setOpen(false)} style={{ background: "var(--ds-surface)", color: "var(--ds-ink)", border: "1px solid var(--ds-line)", padding: "2rem" }}>
        <button onClick={() => setOpen(false)} aria-label="Закрыть" style={{ float: "right", background: "transparent", color: "inherit", border: 0 }}><X /></button>
        <h2 id="new-contract" className="ds-display">НОВЫЙ КОНТРАКТ</h2>
        <label>Название<input style={{ display: "block", margin: ".5rem 0 1rem", width: "100%" }} placeholder="Например, кабинет партнёра" /></label>
        <label>Результат для проверки<textarea style={{ display: "block", marginTop: ".5rem", width: "100%" }} rows={4} /></label>
      </dialog>
    </main>
  )
}
