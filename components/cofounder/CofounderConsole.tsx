  ...
  return (
    <main className="ds-body cofounder-cosmos" style={{ minHeight: "100vh", padding: "clamp(1rem, 4vw, 4rem)" }}>
      ...
      <section className="ds-hull ds-glass ds-command-panel" aria-labelledby="command-title">
        ...
        <div className="ds-command-examples" aria-label="Command examples">{commandExamples.map((example) => <button key={example} type="button" className="ds-command-example ds-focus" onClick={() => setCommandText(example)} disabled={!compileResult || commandBusy} title={`Use command: ${example}`}>{example}</button>)}</div>
