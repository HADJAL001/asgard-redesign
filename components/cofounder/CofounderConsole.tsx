...
const commandExamples = ["сделай карточки плотнее", "сделай мобильную версию", "добавь Stripe"] as const
...
export function CofounderConsole() {
  ...
  async function loadEvidence(id: string) {
    const [evidenceResult, qualityResult, commentsResult, diagnosticsResult] = await Promise.allSettled([
      ...
      fetch(`/api/design/blueprint/${id}/diagnostics`, { cache: "no-store" }),
    ])
    ...
    if (diagnosticsResult.status === "fulfilled" && diagnosticsResult.value.ok) {
      const data = await diagnosticsResult.value.json().catch(() => null)
      if (data?.run && Array.isArray(data.findings)) setDiagnostics(data as DiagnosticsState)
    }
  }
  ...
  async function recheckDiagnostics() {
    if (!compileResult) return
    setRecheckingDiagnostics(true)
    try {
      const response = await fetch(`/api/design/blueprint/${compileResult.id}/diagnostics`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "recheck" }), cache: "no-store" })
      const data = await response.json().catch(() => null)
      if (!response.ok || !data?.run || !Array.isArray(data.findings)) throw new Error("Не удалось повторить Error Intelligence scan")
      setDiagnostics(data as DiagnosticsState)
    } catch (error) {
      setCompileError(error instanceof Error ? error.message : "Не удалось повторить Error Intelligence scan")
    } finally {
      setRecheckingDiagnostics(false)
    }
  }
  ...
  return (
    <main className="ds-body cofounder-cosmos" style={{ minHeight: "100vh", padding: "clamp(1rem, 4vw, 4rem)" }}>
      ...
      <section className="ds-hull ds-glass ds-command-panel" aria-labelledby="command-title">
        ...
        <div className="ds-command-examples" aria-label="Command examples">{commandExamples.map((example) => <button key={example} type="button" className="ds-command-example ds-focus" onClick={() => { setCommandText(example); track("blueprint_command_example_selected", { intent: example.includes("Stripe") ? "stripe" : example.includes("мобильную") ? "mobile" : "dense" }) }} disabled={!compileResult || commandBusy} title={`Use command: ${example}`}>{example}</button>)}</div>
      ...
      <dialog ref={dialogRef} className="ds-contract-dialog" aria-label="НОВЫЙ КОНТРАКТ" aria-labelledby="new-contract" onClose={() => setOpen(false)}>
        ...
        <form onSubmit={submitContract} aria-busy={submitting}>
          ...
          <a className="ds-dialog-secondary ds-focus" href="/integrations" target="_blank" rel="noreferrer" title="Connect or test delivery integrations without leaving this contract">Открыть Integrations</a>
          ...
          {compileResult && diagnostics ? <section className="ds-evidence-ledger" aria-label="Error intelligence"><div className="ds-evidence-ledger__head"><div><span className="ds-utility">ERROR INTELLIGENCE / REVISION {compileResult.revision}</span><small>{diagnostics.run.findingCount ? `${diagnostics.run.findingCount} findings with provenance` : "No findings detected"}</small></div><span data-status={diagnostics.run.status === "passed" ? "passed" : "pending"}>{diagnostics.run.status}</span><button type="button" className="ds-focus ds-evidence-ledger__refresh" onClick={() => void recheckDiagnostics()} disabled={recheckingDiagnostics} title="Run a fresh evidence-bound diagnostics scan" aria-label="Run diagnostics scan"><RefreshCw size={13} aria-hidden="true" className={recheckingDiagnostics ? "ds-spin" : undefined} /></button></div><div className="ds-evidence-ledger__gates" aria-label="Sandbox provenance"><span data-status={diagnostics.run.sandbox.status === "passed" && diagnostics.run.sandbox.verified ? "passed" : diagnostics.run.sandbox.status === "not-run" ? "pending" : "failed"}><i aria-hidden="true" /><strong>Sandbox</strong><em>{diagnostics.run.sandbox.status === "passed" && diagnostics.run.sandbox.verified ? `Verified · ${diagnostics.run.sandbox.durationMs}ms` : diagnostics.run.sandbox.status === "not-run" ? "Not run" : diagnostics.run.sandbox.exitCode === null ? diagnostics.run.sandbox.status : `${diagnostics.run.sandbox.status} · exit ${diagnostics.run.sandbox.exitCode}`}</em></span></div>{diagnostics.history?.length ? <div className="ds-evidence-ledger__gates" aria-label="Diagnostics history">{diagnostics.history.slice(-3).map((entry) => <span key={entry.id} data-status={entry.sandbox.verified ? "passed" : entry.status === "blocked" ? "failed" : "pending"}><i aria-hidden="true" /><strong>Rev {entry.revision}</strong><em>{entry.status} · {entry.sandbox.status}</em></span>)}</div> : null}{diagnostics.findings.length ? <ul>{diagnostics.findings.map((finding) => <li key={finding.id}><i data-status={finding.severity === "high" ? "failed" : "pending"} aria-hidden="true" /><span><strong>{finding.title}</strong><small>{finding.detail} · {finding.source} · confidence {Math.round(finding.confidence * 100)}%</small></span><em>{finding.nextAction}</em></li>)}</ul> : <p className="ds-evidence-ledger__ready">Контракт, evidence и approval room не имеют активных findings.</p>}<small className="ds-field-hint">Auto-repair ограничен детерминированными классами; максимум попыток: {diagnostics.policy.maxRepairAttempts}. Все остальные изменения требуют explainable diff и approval.</small></section> : null}
