...
import { Check, Monitor, Pencil, Smartphone, Sparkles } from "lucide-react"
...
export function BlueprintCanvas({ plan, productType, preset, onCreate }: { plan: BlueprintCanvasPlan | null; productType: ProductType; preset: VisualPreset; onCreate: () => void }) {
  ...
  function updateRole(id: string, role: string) {
    setSlots((current) => current.map((slot) => slot.id === id ? { ...slot, role } : slot))
    setDirty(true)
  }
  ...
  return (
    <section className="ds-blueprint-canvas ds-glass ds-hull" aria-label="Live product preview">
      ...
      <footer className="ds-blueprint-canvas__foot"><span>{plan ? `${slots.length} editable blocks · ${plan.stages.length} delivery stages` : "No blueprint yet"}</span><span>{dirty ? "Draft changes · not saved" : viewport === "desktop" ? "Desktop 1440" : "Mobile 390"}</span></footer>
    </section>
  )
}
