        ...
        <div>
          ...
          <div className="ds-catalog-grid" role="listbox" aria-label="Product type">
            {products.map(({ id, label, detail, Icon }) => {
              ...
              return <button key={id} type="button" role="option" tabIndex={selected ? 0 : -1} aria-selected={selected} aria-label={`Choose ${label}: ${detail}`} title={`Choose ${label} product`} className="ds-catalog-card ds-catalog-card--holo ds-focus" data-selected={selected} onPointerMove={(event) => { const rect = event.currentTarget.getBoundingClientRect(); event.currentTarget.style.setProperty("--tilt-x", `${((event.clientY - rect.top) / rect.height - .5) * -4}deg`); event.currentTarget.style.setProperty("--tilt-y", `${((event.clientX - rect.left) / rect.width - .5) * 4}deg`) }} onPointerLeave={(event) => { event.currentTarget.style.removeProperty("--tilt-x"); event.currentTarget.style.removeProperty("--tilt-y") }} onKeyDown={(event) => moveSelection(event, products.map((product) => product.id), productType, onProductTypeChange)} onClick={() => onProductTypeChange(id)}><span className="ds-catalog-icon"><Icon size={18} aria-hidden="true" /></span><span><strong>{label}</strong><small>{detail}</small></span>{selected ? <Check size={15} aria-hidden="true" /> : null}</button>
            })}
          </div>
        </div>
        <div>
          ...
          <div className="ds-catalog-presets" role="radiogroup" aria-label="Visual DNA">
            {presets.map(({ id, label, detail, color }) => {
              ...
              return <button key={id} type="button" role="radio" tabIndex={selected ? 0 : -1} aria-checked={selected} aria-label={`Choose ${label} visual style: ${detail}`} title={`Use ${label} visual style`} className="ds-catalog-preset ds-focus" data-selected={selected} onKeyDown={(event) => moveSelection(event, presets.map((presetItem) => presetItem.id), preset, onPresetChange)} onClick={() => onPresetChange(id)}><i className={`ds-dna-preview ds-dna-${id}`} aria-hidden="true" style={{ "--dna-color": color } as CSSProperties} /><span><strong>{label}</strong><small>{detail}</small></span>{selected ? <Check size={15} aria-hidden="true" /> : null}</button>
            })}
          </div>
        </div>
      </div>
