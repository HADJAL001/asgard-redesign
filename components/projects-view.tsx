    ...
    <div className="relative min-h-screen overflow-hidden font-sans" style={{ background: "linear-gradient(180deg, #0A0A0F 0%, #14141E 100%)", color: COLORS.text }}>
      ...
      <SectionHelp
        ...
        goals={[
          ...
          { goal: "Попасть внутрь проекта", steps: ["Кликните по карточке — откроется Мастерская", "Конвейер сверху показывает: Замысел → DeepSeek пишет код → Компилятор → Запуск → Деплой", "Кнопка «Запустить» поднимает приложение прямо во вкладке браузера"] },
        ]}
        tour={[
