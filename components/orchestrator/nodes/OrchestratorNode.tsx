    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {palette && <span className="orch-node-icon" style={{ color: data.status === "running" ? COLORS.accent : palette.color }}><palette.Icon size={18} /></span>}
          <span
...
.orch-node { position: relative; border-radius: 9px; backdrop-filter: blur(18px); box-shadow: inset 0 1px rgba(255,255,255,.12), 0 14px 32px rgba(0,0,0,.35); transition: transform .2s ease, border-color .2s ease, box-shadow .2s ease; }
.orch-node:hover { transform: translateY(-2px); box-shadow: inset 0 1px rgba(255,255,255,.16), 0 18px 38px rgba(0,0,0,.48); }
.orch-node-icon { display:grid; width:30px; height:30px; flex:0 0 auto; place-items:center; border:1px solid currentColor; border-radius:7px; background:rgba(255,255,255,.05); box-shadow:inset 0 0 14px rgba(255,255,255,.04); }
.orch-node::before { content:""; position:absolute; inset:0; border:1px solid rgba(180,220,255,.08); border-radius:inherit; pointer-events:none; }
...
@keyframes orch-glow-pulse {
  0%, 100% { box-shadow: 0 0 0 1px rgba(94,216,255,.24), 0 0 14px rgba(94,216,255,.25); }
  50%       { box-shadow: 0 0 0 2px rgba(94,216,255,.5), 0 0 22px rgba(94,216,255,.48); }
}
.orch-node-running {
