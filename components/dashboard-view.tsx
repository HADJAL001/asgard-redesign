...
export function DashboardView() {
  ...
  /* Проверяем прогресс онбординга при монтировании дашборда.
     Если onboarding_step < 5 → показываем туториал OnboardingTutorial. */
  useEffect(() => {
    let cancelled = false
    ;(async () => {
      try {
        ...
        if (!cancelled) {
          setOnboardingStep(data.completed ? null : data.currentStep)
        }
      } catch {
        /* если статус не удалось получить — просто не показываем туториал */
      }
    })()
    return () => {
      cancelled = true
