            ...
            <article className="silver-card">
              <div className="silver-inner flex h-56 flex-col justify-end p-4">
                <div className="pointer-events-none absolute inset-x-0 top-2 flex items-center justify-center">
                  <div className="aspect-square w-40 rounded-full [box-shadow:0_0_80px_rgba(215, 174, 87,0.15)]">
                    <Globe3D />
                  </div>
                </div>
    ...
    <div className={`min-h-screen pt-20 ${activeNav === "Разработчик" ? "elite-chrome" : ""}`}>
      ...
      <div className="mx-auto flex max-w-[1480px] flex-col gap-6 p-5 md:p-8">
        ...
        <section className="mx-auto w-full max-w-2xl">
          ...
          <div className="relative overflow-hidden rounded-2xl shadow-[0_0_44px_-14px_rgba(215,174,87,0.45)] ring-1 ring-white/5">
            ...
            <div className="relative z-10 flex flex-col gap-2.5 p-3">
              ...
              <div className="flex items-center gap-2.5 rounded-xl bg-[rgb(14_16_22)] px-3.5 py-2.5">
                <Search className="size-4 shrink-0 text-amber-300 drop-shadow-[0_0_6px_rgba(215,174,87,0.7)]" />
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  {[
                    ...
                    <button
                      ...
                      type="button"
                      aria-label={label}
                      className="grid size-8 place-items-center rounded-md border border-white/10 bg-white/5 text-slate-300 transition-colors duration-200 hover:border-amber-400/40 hover:text-amber-300"
                    >
                ...
                <button
                  ...
                  style={{ background: "rgba(215, 174, 87,0.08)", border: "1px solid rgba(215, 174, 87,0.4)", color: "#d7ae57" }}
                  onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(215, 174, 87,0.15)"; e.currentTarget.style.borderColor = "rgba(215, 174, 87,0.65)" }}
                  onMouseLeave={(e) => { e.currentTarget.style.background = "rgba(215, 174, 87,0.08)"; e.currentTarget.style.borderColor = "rgba(215, 174, 87,0.4)" }}
                >
  ...
  return (
    <div className="flex shrink-0 items-center gap-2">
      ...
      <div className="flex w-11 flex-col gap-1.5" aria-hidden="true">
        {bars.map(({ label, pct }) => (
          <div key={label}>
            <span className="block font-mono text-[7px] font-semibold uppercase tracking-[0.16em] text-amber-300/70">{label}</span>
            <div className="mt-0.5 h-[3px] w-full overflow-hidden rounded-full bg-amber-500/10">
              <div className="h-full rounded-full bg-amber-400 shadow-[0_0_8px_#d7ae57]" style={{ width: `${pct}%` }} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="relative grid size-20 shrink-0 place-items-center">
        <svg viewBox="0 0 100 100" className="!size-full -rotate-90">
          ...
          <circle cx="50" cy="50" r={r} fill="none" stroke="#d7ae57" strokeWidth="3.5" strokeDasharray={`${seg} ${gap}`} strokeLinecap="round" style={{ filter: "drop-shadow(0 0 3px #d7ae57)" }} />
        ...
        {/* inner glass sphere */}
          <div className="pointer-events-none absolute left-1/2 top-1/2 size-[56%] -translate-x-1/2 -translate-y-1/2 rounded-full border border-amber-500/30 bg-amber-950/70" aria-hidden="true" />

        {/* gauge core readout */}
        <div className="absolute inset-0 flex flex-col items-center justify-center leading-none">
          <span className="font-display text-lg font-bold tracking-tight text-amber-50 drop-shadow-[0_0_6px_rgba(215,174,87,0.7)]">94%</span>
          <span className="mt-1 rounded-full border border-amber-400/40 bg-amber-500/10 px-1.5 py-px font-mono text-[7px] font-bold tracking-wider text-amber-200">12/12</span>
        </div>

        {/* framing corner brackets */}
        <span className="pointer-events-none absolute left-0 top-0 size-2.5 border-l border-t border-amber-400/50" aria-hidden="true" />
        <span className="pointer-events-none absolute right-0 top-0 size-2.5 border-r border-t border-amber-400/50" aria-hidden="true" />
        <span className="pointer-events-none absolute bottom-0 left-0 size-2.5 border-b border-l border-amber-400/50" aria-hidden="true" />
        <span className="pointer-events-none absolute bottom-0 right-0 size-2.5 border-b border-r border-amber-400/50" aria-hidden="true" />
      </div>
    </div>
  )
}
    ...
    <section ref={stageRef} className="neural-stage" aria-label="Digital Nervous System">
      ...
      <div className="neural-node flex flex-col gap-4" {...nodeProps("profile")}>
        <div className="flex items-center justify-between">
          ...
          <span className="flex items-center gap-1.5 text-[10px] text-amber-300"><span className="size-1.5 rounded-full bg-amber-400 shadow-[0_0_6px_#d7ae57]" /> Online</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            { label: "Synergy", value: "94%", Icon: Activity, text: "text-amber-300", border: "border-amber-400/30", bg: "bg-amber-400/[0.07]", label2: "text-amber-300/80", icon: "text-amber-400" },
      ...
      <div className="neural-node" {...nodeProps("credits")}>
        <div className="flex items-center gap-3">
          <CircularDial value={78} max={100} size={52} color="#22d3ee" track="rgba(215,174,87,0.14)">
            <span className="font-display text-[11px] font-bold text-amber-200"><CountUp end={78} suffix="%" /></span>
          </CircularDial>
          <div className="min-w-0">
      ...
      <div className="neural-node" {...nodeProps("uptime")}>
        ...
        <p className="font-display text-xl font-bold text-amber-300"><CountUp end={99.9} decimals={1} suffix="%" /></p>
      ...
      <div className="neural-node" {...nodeProps("analytics")}>
        <div className="mb-2 flex items-center justify-between"><h3 className="neural-node-title font-display text-sm font-semibold">Analytics</h3><span className="flex items-center gap-0.5 font-display text-sm font-bold text-amber-300"><ArrowUp className="size-3.5" />+18%</span></div>
...
function UserMatrixWidget() {
  return (
    <div className="hud-widget p-4">
      <div className="flex items-center justify-between">
        <p className="font-mono text-[10px] uppercase tracking-[0.24em] text-amber-300/80">User Matrix</p>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {["ONLINE", "SECURE", "SYNCED"].map((s, i) => (
          <span key={s} className="inline-flex items-center gap-1 rounded-full border border-amber-500/25 bg-amber-950/30 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-amber-300">
            <span className="hud-blink size-1.5 rounded-full bg-amber-400 shadow-[0_0_6px_rgba(215,174,87,0.9)]" style={{ animationDelay: `${i * 0.4}s` }} /> {s}
          </span>
        ))}
      </div>
    </div>
...
function AtmosphereWidget() {
  const rows = [
    ...
    { label: "A.I. Synergy", value: "94%", note: "Optimal", pct: 94, color: "#d7ae57", icon: Activity, warn: false },
...
function DeveloperDashboard() {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center px-4 py-10">
      {/* Console window — 60% x 70%, centered, transparent, 1px #30424b border, 8px radius */}
      <div
        ref={frameRef}
        onMouseMove={handleMove}
        onMouseLeave={handleLeave}
        className="group relative flex h-[70vh] w-[60vw] min-w-[320px] flex-col overflow-hidden"
        style={{ background: "transparent", border: "1px solid #30424b", borderRadius: 8, perspective: "1200px" }}
      >
        ...
        <div className="relative z-20 grid grid-cols-3 gap-4 border-t p-6 pt-4 md:px-8" style={{ borderColor: "rgba(26,42,74,0.8)" }}>
          {/* PROJECT */}
          <div>
            <p className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.18em]" style={{ color: "#9eb2bc" }}>
            ...
            <p className="mt-2 flex items-center gap-2 font-mono text-[11px] text-slate-300">
              <Clock size={14} strokeWidth={1.5} style={{ color: "#9eb2bc" }} className="opacity-50" />
              284 days
            </p>
            <p className="mt-1 flex items-center gap-2 font-mono text-[11px] text-slate-500">
              <TrendingUp size={14} strokeWidth={1.5} style={{ color: "#9eb2bc" }} className="opacity-50" />
              78% / year
            </p>
          </div>
          {/* USER MATRIX */}
          <div>
            ...
            <p className="mt-2 flex items-center gap-2 font-mono text-[11px] text-slate-300">
              <User size={14} strokeWidth={1.5} style={{ color: "#9eb2bc" }} className="opacity-50" />
              1,289
            </p>
            <p className="mt-1 flex items-center gap-2 font-mono text-[11px] text-slate-500">
              <Circle size={9} strokeWidth={1.5} fill="#9eb2bc" style={{ color: "#9eb2bc" }} className="opacity-60" />
              ONLINE
            </p>
            <p className="mt-1 flex items-center gap-2 font-mono text-[11px] text-slate-500">
              <Shield size={14} strokeWidth={1.5} style={{ color: "#9eb2bc" }} className="opacity-50" />
              SECURE
            </p>
            <p className="mt-1 flex items-center gap-2 font-mono text-[11px] text-slate-500">
              <RefreshCw size={14} strokeWidth={1.5} style={{ color: "#9eb2bc" }} className="opacity-50" />
              SYNCED
            </p>
          </div>
          {/* GENERATION RATRIX */}
          <div>
            ...
            <div className="mt-2 space-y-1 font-mono text-[11px] text-slate-400">
              <p className="flex items-center gap-2">
                <Globe size={14} strokeWidth={1.5} style={{ color: "#9eb2bc" }} className="shrink-0 opacity-50" />
                <span className="flex-1">WEBAPP</span>
                <span className="text-slate-200">64%</span>
              </p>
              <p className="flex items-center gap-2">
                <Brain size={14} strokeWidth={1.5} style={{ color: "#9eb2bc" }} className="shrink-0 opacity-50" />
                <span className="flex-1">NEURALINE</span>
                <span className="text-slate-200">24%</span>
              </p>
              <p className="flex items-center gap-2">
                <Link2 size={14} strokeWidth={1.5} style={{ color: "#9eb2bc" }} className="shrink-0 opacity-50" />
