...
import { foresightReport, preflight } from "../lib/generation-preflight"
...
router.get("/platform-memory", requireAuth, (_req: AuthRequest, res) => {
  ...
  res.json({
    ...
    /* --- Взгляд наперёд (волна 7, п.4) ---
       Всё выше — память о прошлом. Это единственное поле про будущее: у скольких
       проектов выведен класс продукта и по скольким классам фактов хватает на вывод.

       Без него «платформа видит наперёд» проверить нечем. В проде возможны оба
       вырожденных случая: миграция 101 не доехала (класс выведен у нуля проектов) или
       выведена у всех, но заявки не описывают функцию (`unknownClass` близко к
       `classified`) — тогда механизм честно отвечает «фактов нет» на каждую заявку, и
       это должно быть видно числом, а не выясняться расследованием. */
    foresight: foresightReport(),
  })
})

/* ---------------- POST /projects/preflight — что платформа видит в заявке ДО генерации ----------------
   Волна 7, п.4. До этого роута платформа не смотрела на заявку вообще: проверялось лишь
   «имя ИЛИ идея непусты», а первый осмысленный взгляд случался внутри генерации — когда
   время и деньги уже потрачены.

   Отвечает на три вопроса: что за продукт просят (класс, а не тема из словаря), на что
   это похоже из прошлых генераций и чем те кончились, что в заявке не определено и чем
   это грозит. Разбор — lib/generation-preflight.

   МЕТОД POST ПРИ ЧИТАЮЩЕЙ ПРИРОДЕ — намеренно: заявка это свободный текст на несколько
   килобайт, в query-строке он ломается о лимиты и о логи прокси, где остался бы целиком.
   Ни одной записи в БД здесь нет и быть не должно — человек ещё не решил генерировать.

   НИ ОДНОГО ОБРАЩЕНИЯ К МОДЕЛИ: запрет доски («не превращать это в лишний AI-вызов»)
   выполняется тем, что вызывать нечего — класс и пробелы это чистые функции, похожесть
   и исходы это SQL. Стоимость и время генерации не меняются.
--------------------------------------------------------------------------------------- */
router.post("/preflight", requireAuth, (req: AuthRequest, res) => {
  const { name, hint } = req.body || {}
  const safeName = typeof name === "string" ? name.slice(0, 400) : ""
  const safeHint = typeof hint === "string" ? hint.slice(0, 4000) : ""

  /* Пустая заявка — не ошибка: окно создания проекта открывается пустым, и взгляд
     обязан обновляться по мере набора текста. На пустом входе честно вернётся
     «класс не определён» и пробел «заявка слишком короткая». */
  res.json({ preflight: preflight({ userId: req.user!.userId, name: safeName, hint: safeHint }) })
})
