...
const LESSON_TEXT: Record<string, string> = {
  "review-data-contract":
    "для каждого серверного сценария держи согласованный контракт данных: добавь идемпотентную SQL-схему и миграции, используй корректные параметризованные запросы, возвращай человекочитаемые связанные значения и обновляй timestamps во всех UPDATE",
  "review-feature-completeness":
    "реализуй весь запрошенный продуктовый поток, а не только статический экран: навигация, состояния загрузки/ошибки/пусто, рабочие действия и экспорт должны быть связаны с данными и доступны из интерфейса",
  "review-design-contract":
    "соблюдай согласованный дизайн-контракт буквально: используй заявленные токены палитры, типографику, плотность и адаптивные состояния вместо случайной цветовой схемы",
  "review-billing-flow":
    "если приложение заявлено как подписочное, реализуй видимое состояние тарифа, биллинга, лимитов и мест; кнопка оплаты должна вести в реальный согласованный поток, а не быть декоративной",
  "review-runtime-behavior":
    "проверяй поведение за пределами компиляции: каждое действие интерфейса должно иметь рабочий обработчик, API-ответ должен соответствовать типам и после мутаций данные должны оставаться согласованными",
  "use-client-missing":
