...
/**
 * Сколько генераций случилось начиная с момента `sinceMs` (включительно).
 *
 * Знаменатель для всего, что считается «на генерацию»: частота возврата дефекта и
 * затухание урока (`lib/lesson-decay`). Живёт здесь, а не в корпусе ремесла, потому
 * что журнал генераций принадлежит этому модулю — считать его строки из двух мест
 * значит рано или поздно разойтись в определении слова «генерация».
 *
 * `null` — знаменателя НЕ СУЩЕСТВУЕТ: схемы без миграции 094 или пустого журнала.
 * Это принципиально не ноль: ноль генераций означал бы деление на ноль и соблазн
 * подставить единицу, то есть выдумать трафик, которого не было. Вызывающий обязан в
 * этом случае вести себя как до волны 7 — по абсолютным числам.
 */
export function generationsSince(sinceMs: number | null): number | null {
  try {
    const row =
      sinceMs === null
        ? (db.prepare(`SELECT COUNT(*) as n FROM generation_learning`).get() as { n: number })
        : (db.prepare(`SELECT COUNT(*) as n FROM generation_learning WHERE created_at >= ?`).get(sinceMs) as {
            n: number
          })
    /* Пустой журнал — это «мерить нечем», а не «ноль генераций»: строка появляется на
       каждой генерации, поэтому её отсутствие говорит только об отсутствии измерения. */
    return row.n > 0 ? row.n : null
  } catch {
    return null // схема без 094 — знаменателя нет, и подменять его нельзя
  }
}
