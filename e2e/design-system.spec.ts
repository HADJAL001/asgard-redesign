...
test.describe("OSGARD design system", () => {
  ...
  test("binds evidence ledger entries to the contract hash", async ({ request }) => {
    ...
    expect(generationBody.history).toHaveLength(1)
    const generationRead = await request.get(`/api/design/blueprint/${blueprint.id}/generation`)
    expect(generationRead.status()).toBe(200)
    const generationReadBody = await generationRead.json()
    expect(generationReadBody.generation.result.appUrl).toBe("https://example.com/app")
    expect(generationReadBody.history).toHaveLength(1)
    expect(await generationRead.text()).not.toContain(createdBody.evidenceToken)
    const graphResponse = await request.get(`/api/design/blueprint/${blueprint.id}/graph`)
    expect(graphResponse.status()).toBe(200)
    const graphBody = await graphResponse.json()
    expect(graphBody.graph).toMatchObject({ blueprintId: blueprint.id, tenantId: "osgardnewworld" })
    expect(graphBody.graph.nodes.map((node: { kind: string }) => node.kind)).toEqual(expect.arrayContaining(["idea", "contract", "evidence", "generation"]))
    expect(graphBody.graph.edges.map((edge: { kind: string }) => edge.kind)).toEqual(expect.arrayContaining(["created", "verified_by", "generated_as"]))
    expect(await graphResponse.text()).not.toContain(createdBody.evidenceToken)
  ...
  test("persists a tenant-bound delivery policy before deploy", async ({ request }) => {
    ...
    expect(policyBody.preflight).toMatchObject({ ready: true })
    expect(policyBody.preflight.checks).toEqual(expect.arrayContaining([expect.objectContaining({ id: "domain", status: "manual" }), expect.objectContaining({ id: "supabase", status: "passed" })]))
  ...
  test("cofounder renders hull workspace with keyboard-visible controls", async ({ page }) => {
    ...
    const canvas = page.getByRole("region", { name: "Live product preview" })
    await expect(canvas).toBeVisible()
    await expect(canvas.getByRole("button", { name: "Показать mobile preview" })).toHaveAttribute("aria-pressed", "false")
    await canvas.getByRole("button", { name: "Показать mobile preview" }).click()
    await expect(canvas.getByRole("button", { name: "Показать mobile preview" })).toHaveAttribute("aria-pressed", "true")
    const dock = page.getByRole("navigation", { name: "Основная навигация платформы" })
    await expect(dock).toBeVisible()
    await expect(dock.locator(".osgard-dock__item")).toHaveCount(12)
    await expect(dock.getByRole("link", { name: "Кузница" })).toHaveAttribute("title", "Кузница")
    const delivery = page.getByRole("region", { name: "Этапы создания продукта" })
    await expect(delivery.locator(".ds-sequence__stage")).toHaveCount(5)
    await expect(delivery.locator(".ds-sequence__stage--active").first()).toContainText("Идея")
    ...
    const contractName = page.getByRole("textbox", { name: "Название" })
    ...
    const undo = canvas.getByRole("button", { name: "Undo canvas change" })
    const redo = canvas.getByRole("button", { name: "Redo canvas change" })
    await expect(undo).toBeEnabled()
    await expect(redo).toBeDisabled()
    await undo.click()
    await expect(canvas).not.toContainText("Primary mission block")
    await expect(redo).toBeEnabled()
    await redo.click()
    await expect(canvas).toContainText("Primary mission block")
  ...
  test("cofounder offers an accessible preview retry after a transient preview outage", async ({ page }) => {
    ...
    await page.goto("/cofounder")
    await page.getByTitle("Open the product brief and start a verified blueprint").click()
    await page.getByRole("button", { name: "Use starter mission: Launch a product" }).click()
    await page.locator("dialog input[required]").fill("Preview recovery")
    await page.locator("dialog form button[type=submit]").click()
