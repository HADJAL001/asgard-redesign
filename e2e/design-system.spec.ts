...
test.describe("OSGARD design system", () => {
  ...
  test("cofounder renders hull workspace with keyboard-visible controls", async ({ page }) => {
    ...
    await undo.click()
    await expect(canvas.getByRole("textbox", { name: /Edit .* label/ })).not.toHaveValue("Primary mission block")
    await expect(redo).toBeEnabled()
    await redo.click()
    await expect(canvas.getByRole("textbox", { name: /Edit .* label/ })).toHaveValue("Primary mission block")
