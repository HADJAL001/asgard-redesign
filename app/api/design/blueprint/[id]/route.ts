...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ blueprint, revisions: listBlueprintRevisions(id).map(({ revision: value, generatedAt }) => ({ revision: value, generatedAt })) }, { headers: { "cache-control": "no-store" } })
}
