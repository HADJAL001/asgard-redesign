...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!verifyBlueprintEvidenceToken(id, body.evidenceToken)) return NextResponse.json({ error: "invalid_evidence_token" }, { status: 403 })
  ...
  const latest = listBlueprintRevisions(id).at(-1)
  ...
  const evidence = appendBlueprintEvidence({ id: crypto.randomUUID(), blueprintId: id, revision: edited.revision, contractHash, kind: "rollback", status: "passed", summary: `Canvas draft saved as revision ${edited.revision}; approval and quality evidence reset`, capturedAt: new Date().toISOString(), source: "blueprint-canvas-edit" })
