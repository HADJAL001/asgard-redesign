...
async function scan(request: NextRequest, id: string) {
  ...
  const run = { id: `error-run:${id}:${blueprint.revision}`, blueprintId: id, revision: blueprint.revision, contractHash: blueprint.contractHash, status: findings.some((finding) => finding.blocking && finding.severity === "high") ? "blocked" : findings.length ? "needs-review" : "passed", findingCount: findings.length, phases: ["contract", "evidence", "delivery", "approval"].map((phase) => ({ phase, status: findings.some((finding) => finding.category === phase || (phase === "contract" && finding.category === "quality")) ? "attention" : "passed" })) }
  return NextResponse.json({ version: "1.1.0", run, findings, policy: { autoRepair: ["missing-accessible-label", "deterministic-import", "type-error", "formatting"], maxRepairAttempts: 3, approvalRequiredFor: ["security", "data-migration", "provider", "infrastructure"], repairMode: "proposal-only" } }, { headers: { "cache-control": "no-store" } })
}
