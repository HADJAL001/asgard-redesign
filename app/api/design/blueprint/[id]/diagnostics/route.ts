...
async function scan(request: NextRequest, id: string) {
  ...
  let sandboxSummary: { status: "not-run" | "unverified" | "passed" | "failed" | "timeout" | "unavailable"; exitCode: number | null; timedOut: boolean; durationMs: number; verified: boolean } = { status: "not-run", exitCode: null, timedOut: false, durationMs: 0, verified: false }
  ...
  if (generation?.status === "completed" && (!generation.sandbox || generation.sandbox.status === "unavailable")) add({ fingerprint: "sandbox.result-unverified", category: "sandbox", severity: "high", confidence: 1, title: "Build result has no verified sandbox record", detail: "The provider completed the task, but no isolated build exit code and timeout record was returned.", source: "sandbox-provenance", nextAction: "review", blocking: true, autoRepairEligible: false, evidenceRefs: [] })
  if (generation?.sandbox && (generation.sandbox.status === "failed" || generation.sandbox.status === "timeout")) add({ fingerprint: `sandbox.${generation.sandbox.status}`, category: "sandbox", severity: "high", confidence: 1, title: `Sandbox ${generation.sandbox.status}`, detail: generation.sandbox.logTail || `Sandbox reported ${generation.sandbox.status}.`, source: "sandbox-provenance", nextAction: "review", blocking: true, autoRepairEligible: false, evidenceRefs: [] })
  if (generation?.status === "completed") {
    ...
    sandboxSummary = generation.sandbox ? { status: generation.sandbox.status, exitCode: generation.sandbox.exitCode, timedOut: generation.sandbox.timedOut, durationMs: generation.sandbox.durationMs, verified: generation.sandbox.status === "passed" && sealed } : { status: "unverified", exitCode: null, timedOut: false, durationMs: 0, verified: false }
